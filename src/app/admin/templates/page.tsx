...
export default function TemplatesAdmin() {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      ...
      setFormData((prev) => ({ ...prev, frameImage: dataUrl, slotsLayout: [], slots: 0 }));
      removeChromaKey(dataUrl).then((processed) => {
        setImagePreview(processed);
        setFormData((prev) => ({ ...prev, frameImage: processed }));
        const detImg = new window.Image();
        detImg.onload = () => {
          const detected = detectTransparentSlots(detImg);
          if (detected.length > 0) {
            setFormData((prev) => ({ ...prev, slotsLayout: detected, slots: detected.length }));
          }
        };
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (resizeHandle) {
      ...
      setResizing({ index, handle, startX: e.clientX, startY: e.clientY, slot: { ...slot } });
      return;
    }
    const rect = containerRef.current.getBoundingClientRect();
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (resizing) {
      ...
      let newSlot = { ...slot };
      const minSize = 3;
      if (handle.includes('e')) newSlot.w = Math.max(minSize, slot.w + dx);
      if (handle.includes('w')) { const nw = Math.max(minSize, slot.w - dx); newSlot.x = slot.x + (slot.w - nw); newSlot.w = nw; }
      if (handle.includes('s')) newSlot.h = Math.max(minSize, slot.h + dy);
      if (handle.includes('n')) { const nh = Math.max(minSize, slot.h - dy); newSlot.y = slot.y + (slot.h - nh); newSlot.h = nh; }
      setFormData((prev) => {
    ...
    const x = ((e.clientX - rect.left) / rect.width) * 100;
    const y = ((e.clientY - rect.top) / rect.height) * 100;
    setCurrentPos({ x: Math.max(0, Math.min(100, x)), y: Math.max(0, Math.min(100, y)) });
  };
  ...
  const handleMouseUp = () => {
    ...
    if (w > 1.5 && h > 1.5) {
      const newSlot: ISlot = { x: Math.round(x * 10) / 10, y: Math.round(y * 10) / 10, w: Math.round(w * 10) / 10, h: Math.round(h * 10) / 10 };
      setFormData((prev) => {
        const updated = [...(prev.slotsLayout || []), newSlot];
        return { ...prev, slotsLayout: updated, slots: updated.length };
      });
    }
  };
        ...
        <div className={`glass-panel ${styles.formCard}`}>
          ...
          <form onSubmit={handleSave}>
              ...
              <div className={styles.formFields}>
                <div className={styles.formRow}>
                  <div className={styles.formGroup}>
                    ...
                    <input type="text" required value={formData.templateId} onChange={(e) => setFormData({ ...formData, templateId: e.target.value })} placeholder="e.g. t4" disabled={!!editingId} />
                  </div>
                  <div className={styles.formGroup}>
                    <label>Name</label>
                    <input type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. Polaroid" />
                  </div>
                </div>
                <div className={styles.formGroup}>
                  <label>Description</label>
                  <textarea required value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Describe the template..." />
                </div>
                <div className={styles.formRow}>
                  <div className={styles.formGroup}>
                    <label>Slots (Shots) - Auto calculated</label>
                    <input type="number" required min="0" max="10" readOnly value={formData.slots} style={{ background: '#f0f0f0', color: '#999', cursor: 'not-allowed' }} placeholder="Upload image and configure slots" />
                  </div>
                  <div className={styles.formGroup}>
                ...
                <div className={styles.formRow}>
                  <div className={styles.formGroup}>
                    <label>Color</label>
                    <input type="color" value={formData.color} onChange={(e) => setFormData({ ...formData, color: e.target.value })} style={{ width: 48, height: 48, padding: 0, border: '2px solid #ddd', borderRadius: 12, cursor: 'pointer', background: 'none' }} />
                  </div>
                  <div className={styles.formGroup}>
                    <label>Status</label>
              ...
              <div className={styles.imagePreviewArea} style={{ gridColumn: 'span 1' }}>
                ...
                {imagePreview ? (
                  <div className={styles.workspaceWrapper}>
                    <div ref={containerRef} className={styles.canvasContainer} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp}>
                      ...
                      {(formData.slotsLayout || []).map((slot, idx) => (
                        <div key={idx} className={styles.slotHighlight} style={{ left: `${slot.x}%`, top: `${slot.y}%`, width: `${slot.w}%`, height: `${slot.h}%` }}>
                          <span className={styles.slotBadge}>Slot {idx + 1}</span>
                          <button type="button" className={styles.slotDeleteBtn} onClick={(e) => { e.stopPropagation(); handleDeleteSlot(idx); }}>×</button>
                      ...
                      {isDrawing && (
                        <div className={styles.activeSelection} style={{ left: `${Math.min(startPos.x, currentPos.x)}%`, top: `${Math.min(startPos.y, currentPos.y)}%`, width: `${Math.abs(startPos.x - currentPos.x)}%`, height: `${Math.abs(startPos.y - currentPos.y)}%` }} />
                      )}
                    </div>

                    <div className={styles.workspaceToolbar}>
                      <button type="button" className="mac-button secondary" onClick={() => setFormData(prev => ({ ...prev, slotsLayout: [], slots: 0 }))}>
                ...
                <div style={{ display: 'flex', gap: '8px', width: '100%' }}>
                  <button type="button" className={`mac-button secondary ${styles.imageUploadBtn}`} onClick={() => fileInputRef.current?.click()} style={{ fontSize: '14px', padding: '10px 16px', flex: 1 }}>
                    <Image size={16} /> Upload Image
                  </button>
                  {imagePreview && (
                    <button type="button" className={`mac-button secondary ${styles.imageUploadBtn}`} onClick={() => { setImagePreview(''); setFormData((prev) => ({ ...prev, frameImage: '', slotsLayout: [], slots: 0 })); }} style={{ fontSize: '14px', padding: '10px 16px', flex: 1 }}>
                      <X size={16} /> Remove
                    </button>
                  )}
                </div>
            ...
            <div className={styles.formActions}>
              <button type="button" className="mac-button secondary" onClick={() => { setShowForm(false); setEditingId(null); }}>Cancel</button>
