...
export default function BoothControls({
  ...
  onBack, handleManualCapture, takePhoto, setCaptureMode, setTaking,
  cameraType, setShowCamMenu, showCamMenu, availableCams, deviceId,
  handleSwitchCamera, isFrontCamera, mirrored, setMirrored, camMenuRef,
}: {
  taking: boolean;
    ...
    <div className={styles.boothControls}>
      {!taking && !dslrCapturing && !busy && (
        <div className={styles.boothBtnRow}>
            <button className={styles.boothBtnSecondary} onClick={onBack} disabled={stripLoading}><ArrowLeft size={16} /> Back</button>
            {captureMode === 'manual' ? (
              <button className={styles.boothBtnPrimary} onClick={handleManualCapture} disabled={filledCount >= slotsCount || busy || stripLoading}>
                {stripLoading ? <Loader2 className="spin" size={22} /> : <CameraIcon size={22} />} {filledCount}/{slotsCount}
              </button>
            ) : (
              <button className={styles.boothBtnPrimary} onClick={() => { setTaking(true); takePhoto(slotsCount - filledCount); }} disabled={filledCount >= slotsCount || busy || stripLoading}>
                {stripLoading ? <Loader2 className="spin" size={22} /> : <CameraIcon size={22} />} {filledCount}/{slotsCount}
              </button>
            )}
                  ...
                  {availableCams.length === 0 ? (
                    ...
                    availableCams.map((cam) => (
                      <button key={cam.deviceId}
                        ...
                        onClick={() => handleSwitchCamera(cam.deviceId)} disabled={stripLoading}>{cam.label || `Camera ${cam.deviceId.slice(0, 8)}...`}</button>
                    ))
                  )}
                </div>
                <button className={styles.boothBtnSecondary} onClick={() => setShowCamMenu((v) => !v)} disabled={stripLoading} title="Ganti kamera">
                  <RefreshCcw size={18} />
                </button>
                {isFrontCamera && (
                  <button className={`${styles.boothBtnSecondary} ${!mirrored ? styles.boothMirrorOff : ''}`}
                    onClick={() => setMirrored((v) => !v)} disabled={stripLoading} title={mirrored ? 'Mirror: ON' : 'Mirror: OFF'}>
