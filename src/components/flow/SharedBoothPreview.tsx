// File: src/components/flow/SharedBoothPreview.tsx
// Description: Auto-added top comment for easier file identification.

'use client';

import { X, Check, Upload, Loader2 } from 'lucide-react';
import type { TemplateData, ISlot } from '@/lib/types';
import { getAutoFormatUrl } from '@/lib/utils/cloudinary-url';
import { compressImage } from '@/lib/utils/image-utils';
import { UPLOAD_MAX_DIM, UPLOAD_JPEG_QUALITY } from '@/lib/utils/constants';
import { useRef } from 'react';

interface SharedBoothPreviewProps {
...
export default function SharedBoothPreview({
  templateData,
  captures,
  keyedFrameImage,
  frameRatio,
  ...
  const handleFileChange = async (idx: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async () => {
      ...
      if (dataUrl) {
        const compressed = await compressImage(dataUrl, UPLOAD_MAX_DIM, UPLOAD_JPEG_QUALITY);
